...
function MovingCloud({ startX, y, z, speed }) {
  ...
  return (
    <group ref={ref} position={[startX, y, z]}>
      <Cloud segments={5} bounds={[4, 1, 1]} volume={4} color="#f0e8e0" opacity={0.65} speed={0} />
    </group>
  )
}
...
// ─── Points counter ───────────────────────────────────────────────────────────
function PointsCounter({ points, bank, onAdd, onSpend, activeTree }) {
  const displayStage = activeTree?.displayStage || 1
  const cost     = getWaterCost(displayStage)
...
export default function TreeScene() {
  ...
  // Each tree: { id, x, z, paletteId, displayStage }
  const [trees, setTrees] = useState([
    { id: 0, x: 0, z: 0, paletteId: 'sakura', displayStage: 1 }
  ])

  const activeTree = trees[activeTreeIdx]
  const allFullyGrown = trees.length > 0 && trees.every(t => t.displayStage === 4)
  ...
  function handleSpend() {
    ...
    if (bank >= cost && activeTree.displayStage < 4) {
      setBank(b => b - cost)
      setWaterState('ready')
    }
  }
  ...
  function handleSelectPalette(paletteId) {
    ...
    // Store pending palette for when user clicks ground
